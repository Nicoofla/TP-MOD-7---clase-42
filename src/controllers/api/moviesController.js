...
const moviesController = {
    ...
    list: async (req, res) => {
        ...
        try {  const movies = await db.Movie.findAll(modelMovie)
            ...
            return res.status(error.status || 500).json({
                ok: false,
                msg: error.message || "Error!! Llamar a Eric"
            })
        }
    },
    ...
    detail: async (req, res) => {
        ...
        try { if (isNaN(req.params.id)) throw createError(404, "Id incorrecto")

            const movie = await db.Movie.findByPk(req.params.id, modelMovie);

            if (!movie) throw createError(404, "no se encuentra la pelicula buscada")
    ...
    create: async function (req, res) {
        ...
        try {  const { title, length, rating, awards, release_date, genre_id } = req.body
    ...
    update: async function (req, res) {
        ...
        try {
            ...
            if ([title, rating, awards, release_date, genre_id, length].includes("" || undefined)) throw createError(400, "Ups!! Todos los campos son obligatorios")
